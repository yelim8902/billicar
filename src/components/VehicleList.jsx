...
import { IconSearch, IconCar, IconChevronLeft, IconMapPin } from './ui/Icon';
...
const LocationCard = styled.div`display:flex;align-items:center;gap:11px;background:#F5F6F7;border-radius:15px;padding:17px;margin-bottom:14px; div{flex:1} b{font-size:15px} p{font-size:13px;margin:5px 0 0;color:${theme.colors.textSecondary}} button{border:0;background:none;color:${theme.colors.primaryDark};font-size:14px;font-weight:700;flex:none;}`;
...
export default function VehicleList({ onSelect, mode = 'default', onBack }) {
  ...
  const { position: userPosition, error: locationError, loading: locating, refresh: refreshLocation } = useUserLocation();

  // 실제 위치 권한이 있으면 하버사인으로 진짜 거리를 계산해서 가까운 순으로 정렬,
  // 없으면 DB/시드 데이터의 표시용 거리 텍스트를 그대로 씀
  const withDistance = useMemo(() => {
    if (!userPosition) return vehicles;
    return vehicles
      .map(v => {
        const km = haversineKm(userPosition, v.coords);
        return km == null ? v : { ...v, distanceKm: km, distance: formatDistance(km) };
      })
      .sort((a, b) => (a.distanceKm ?? Infinity) - (b.distanceKm ?? Infinity));
  }, [vehicles, userPosition]);
    ...
    <Screen>
      ...
      {mode !== 'default' && (
        <LocationCard>
          <IconMapPin size={20} />
          <div>
            <b>{locating ? '위치 확인 중…' : userPosition ? '현재 위치 확인됨' : '위치 정보 없음'}</b>
            <p>{locating ? '잠시만 기다려주세요' : userPosition ? '가까운 차량 순으로 보여드려요' : (locationError || '위치 권한을 허용해주세요')}</p>
          </div>
          <button onClick={refreshLocation}>다시 찾기</button>
        </LocationCard>
      )}
