...
## ⚠️ 프로젝트명 변경: MobiTrust → BilliCar (2026-09-15)

- GitHub 저장소: `mobitrust-frontend` → `billicar` (`gh repo rename`, 기존 URL은 자동 리다이렉트됨)
- 코드/문서 전반의 "MobiTrust"/"Mobi Trust"/"모비" 표기를 "BilliCar"/"Billi Car"/"빌리"로 전부 교체
- DB `handle_new_user()` 함수의 신규 가입자 기본 표시 이름도 새 마이그레이션(202609150008)으로 갱신, 적용 완료
- **바꿀 수 없는 것**: 이미 배포된 `MockWKRW` 컨트랙트는 생성자에서 토큰 이름을 `"Wrapped KRW (MobiTrust Demo)"`로 고정해서 온체인에 새겨놨음 — 이건 컨트랙트 불변이라 코드만 고쳐선 안 바뀜. **재배포해야만** 실제로 "BilliCar Demo"로 보임 (재배포하면 주소도 바뀌어서 `src/contracts/*.json` 다시 채워야 함 — 지금은 굳이 재배포 안 함, 필요하면 나중에)
- 이미 존재하던 원본 마이그레이션 파일(`202609150001_initial_mobitrust_schema.sql` 등)의 파일명/내용은 일부러 안 건드림 — 이미 적용된 마이그레이션 파일을 수정하면 Supabase 쪽 체크섬/이력 추적이 꼬임

## 지금 해야 할 일 (다음 세션이 이어받을 것)

### ✅ End-to-end 실제 테스트 완료 (2026-09-15)

`npm start`로 로컬 서버 띄우고 실제 MetaMask + Kaia Kairos에서 예약→반납까지 전체 사이클을 브라우저로 직접 눌러서 확인함. 온체인 이벤트로 최종 검증:
- `Escrowed`: 485,000 W-KRW 예치 (대여료 375,000 + 보험료 10,000 + 보증금 100,000)
- `Released`: 차주 337,500 (수수료 10% 제외) / 플랫폼 37,500(수수료+보험료) / 렌터 보증금 환불 100,000 — 금액 전부 정확히 맞음
- Supabase `bookings.status`도 confirmed → active → completed로 정상 전이됨

**즉 지금 코드로 데모/시연 가능한 상태.** 남은 건 아래 "시간 없어서 생략한 것들"에 적힌 알려진 제약(특히 반납은 배포 지갑으로만 가능한 부분) 감안해서 시연 시나리오 짜는 것.
...
### 2026-09-15 (Claude) — 6차
- 프로젝트명 MobiTrust → BilliCar로 전면 변경 (GitHub 저장소 rename 포함, 코드/문서/DB 기본값까지 전부 교체)
- 겸사겸사 README.md도 오랫동안 밀려있던 실제 구현 상태(컨트랙트 배포·연동 완료 등) 반영해서 전체 갱신
- 배포된 MockWKRW 컨트랙트의 온체인 토큰 이름은 못 바꿈(재배포 필요) — 위 "프로젝트명 변경" 섹션에 기록
- 린트/테스트 통과 확인 후 커밋+푸시 예정

### 2026-09-15 (Claude) — 5차
- 실제 브라우저(로컬 npm start)에서 팀원과 함께 예약→이용시작→반납까지 전체 플로우 직접 실행해서 검증
- 온체인 이벤트(Escrowed, Released) 값 RPC로 직접 조회해서 금액까지 정확히 맞는지 확인 완료
- 데모 가능한 상태로 확정 — 자세한 건 위 "End-to-end 실제 테스트 완료" 참고
